...
% napsat vo uprave user session -- logged_in = true aby se v tech testech nemuselo furt logovat a starat se vo heslo
Zobrazení testovaných stránek vyžaduje přihlášení do systému. To by vyžadovalo zaslání přihlašovacího požadavku se správným heslem před spuštěním testů.
...
\begin{listing}[htbp]
\caption{\label{lst:logged_in} Explicitní nastavení proměnné \texttt{logged\_in} na požadovanou hodnotu}
\begin{minted}[bgcolor=codebg]{python}

...
\begin{listing}[htbp]
\caption{\label{lst:route} Přiřazení funkce k~URL. Funkce \texttt{index} bude zavolána při každém příchozím požadavku s~metodou \texttt{get} na kořenové URL \texttt{/}. Tato funkce vrací vygenerovanou HTML stránku, která bude zaslána klientovi}
\begin{minted}[bgcolor=codebg]{python}
...
Pokud funkce zpracovávající požadavek mění obsah databáze (například vytváří novou garáž), je vyžadována metoda \texttt{post}.

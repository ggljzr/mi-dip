...
\begin{itemize}
    ...
    \item Systém má umožňovat správu pomocí webového rozhraní s~následujícími funkcemi:
        \begin{itemize}
...
Jiná možnost je použití \textit{self-signed} certifikátu, který není podepsaný žádnou certifikační autoritou, ale pouze vlastníkem certifikátu \cite{cert_wallen}. Může tedy sloužit k~šifrování komunikace (poskytuje veřejný klíč), ale je zranitelný vůči \textit{man-in-the-middle} útoku \cite{cert_wallen}.
...
Podřízené systémy by při zasílání požadavků musely přeskočit krok ověření totožnosti serveru. Jak toho dosáhnout například v~knihovně Requests (umožňující vytváření HTTP požadavků \cite{requests}) pro jazyk Python \cite{python_tutorial} je naznačeno v~ukázce \ref{lst:req_selfsigned}. 
...
Přístup k~API nadřazeného systému by měl být povolen pouze ověřeným klient\-ům. Díky tomu bude možné zabránit například zasílání nepravdivých informací z~neznámých zdrojů.
...
Na obrázku \ref{fig:basic_mqtt} \textcolor{green}{klienti A} a \textcolor{green}{B} začnou odebírat \textit{topic} \uv{Teplota}. Když pak \textcolor{magenta}{klient C} publikuje zprávu na tuto adresu, \textcolor{blue2}{\textit{broker}} se postará o~doručení všem odebírajícím klientům.
...
V~případě této práce by nadřazený systém i podřízené systémy byly klienty \textit{brokeru}. Podřízené systémy by publikovaly naměřená data, která by nadřazený systém odebíral. Tím by vznikla obdoba modelu \textit{client/server}, zmíněného v~sekci \ref{sec:an_struct}. 
...
Kromě certifikátů lze pro šifrování použít i PSK. V~tom případě \textit{broker} a jeho klienti používají pro zašifrování komunikace společný klíč (známý jak klientovi, tak \textit{brokerovi}). Různí klienti přitom mohou mít různé klíče. \cite{mqtt_mosquitto_conf}
...
V~sekcích \ref{sec:an_https} a \ref{sec:an_mqtt} jsem blíže prozkoumal dva poměrně rozšířené protokoly aplikační vrstvy, které by bylo možné použít pro tvorbu nadřazeného systému.

Pokud by mezi požadavky na systém bylo zahrnuto zasílání nevyžádaných zpráv podřízeným systémem (jak je zmíněno v~sekci \ref{sec:an_struct}), zvolil bych pravděpodobně protokol MQTT. V~něm je tato funkcionalita velmi snadno implementovatelná -- stačí, aby podřízené systémy odebíraly \textit{topic}, na kterém by nadřazený systém publikoval zprávy.
...
Identifikátory a typ události pravděpodobně budou 32bitová celá čísla. Datum je možné v~SQLite ukládat buď jako textový retězec s~formátem \texttt{YYYY-MM-DD HH:MM:SS.SSS}, 64bitové reálné číslo, či 32bitové celé číslo -- \textit{Unix Time} \cite{sqlite_datetypes}. Z~toho vyplývá, že jedno kontrolní hlášení bude mít velikost alespoň $4 \cdot 32 = 128$ bitů.

Pokud nadřazený systém monitoruje 30 garáží, přičemž každá odesílá kontrolní hlášení každou hodinu, jsou každý den vytvořeny záznamy o~velikosti $30 \cdot 24 \cdot 96 = 98304$ bitů, tedy asi 98 Kb. Dá se proto předpokládat, že velikost databázového souboru neporoste nijak dramaticky.
...
V~následujících sekcích jsem vyzkoušel několik přístupů k~odesílání těchto notifikací, a to jak pomocí e-mailu, tak SMS.
...
Gmail umožňuje přístup k~účtu pomocí protokolu SMTP, je však nutné v~nastavení účtu Google povolit přístup méně zabezpečeným aplikacím, protože přihlášení je prováděno pomocí uživatelského jména a hesla \cite{google_smtp}. Jelikož by příhlašovací údaje musely být uloženy v~nadřazeném systému v~čitelné podobě, není tento způsob vhodný.

Bezpečnější možnost je použít k~přístupu protokol OAuth2 \cite{rfc6749}. S~jeho pomocí je možné udělovat aplikacím (v~tomto případě nadřazenému systému) přístup k~částem účtu Google, tedy i Gmailu. Autentizace probíhá na základě náhodně generovaných klíčů, takže není nutné uchovávat v~nadřazeném systému heslo, které mohl uživatel systému použít i v~jiných službách, v~čitelné podobě \cite{google_oauth}.
...
K~vytvoření webového rozhraní i API pro podřízené systémy jsem zvolil framework Flask \cite{flask_about}. Hlavním důvodem jsou opět předchozí zkušenosti s~tímto frameworkem. Flask také dává více volnosti při návrhu aplikace než například také velmi rozšířený framework Django \cite{django_about}.
...
V~dalších sekcích jsem blíže zkoumal jednodeskové počítače Raspberry Pi (sekce \ref{sec:an_rpi}) a Zybo Zynq-7000 (sekce \ref{sec:an_zyb}) a zvážil jejich výhody a nevýhody pro implementaci systému.
...
Jako platformu pro implementaci nadřazeného systému jsem zvolil počítač Raspberry Pi 3, především kvůli příznivé ceně, výrazně lepšímu poměru cena/výkon (pro tuto práci), a také kvůli podpoře a rozšiřitelnosti.

Kromě nákladů popsaných v~seckích \ref{sec:an_rpi} a \ref{sec:an_zyb} je ještě potřeba počítat s~pořízením GSM modulu. V~závislosti na typu modulu se jeho cena může pohybovat kolem 800 až 1200 Kč\footnote{Cena LTE modulu Huawei E3372h na e-shopu \url{https://www.t-mobile.cz} v~březnu 2018 je 1151 Kč.}.

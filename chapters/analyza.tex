...
\begin{itemize}
    ...
    \item Systém má umožňovat správu pomocí webového rozhraní, ve kterém je možné:
        \begin{itemize}
        ...
        \end{itemize}
    \item Systém má být schopen nějakým způsobem informovat provozovatele při poplachu.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Prezistence} -- data v~systému by měla být uchovávána prezistentním způsobem.
    \item \textbf{Nezávislost} -- Systém by neměl být příliš závislý na externích službách a alespoň v~omezené fungovat i bez přístupu k~internetu.
\end{itemize}
...
%tady na tim miste bude este neco k~ty sekci vo emailu a notifikacich, kterou dopisu
...
Z~těchto požadavků vyplívá software potřebný k~implementaci nadřazeného systému, jehož dostupnost je hlavní požadavek při volbě hardwarové platformy v~sekci \ref{sec:an_plat}. 
...
Základní požadavek na podřízený systém je schopnost komunikace přes Ethernet či WiFi pomocí protokolu zvoleného v~sekci \ref{sec:an_protocol}, dále pak podpora softwaru popsaného v~sekci \ref{sec:an_plat}. Kromě toho může být hardware prakticky libovolný.

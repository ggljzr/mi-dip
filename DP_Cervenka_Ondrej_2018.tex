...
Nadřazený systém bude se svými klienty (monitorovací zařízení v~jednotlivých garážích) komunikovat přes \verb|WiFi| nebo \verb|Ethernet|. Základem komunikace bude \verb|TCP/IP| protokol, je však potřeba zvolit vhodný protokol z~aplikační vrstvy OSI modelu, který na něm bude stavět.

\subsection{Vlastní protokol}

Jedna z~možností je implementovat vlastní protokol pomocí \verb|TCP/IP| socketů. Toto řešení se mi však nezdá příliš vhodné, neboť nepřináší žádné významné výhody, naopak se s~ním pojí řada komplikací.
...
Další možnost je využít ke komunikaci protokol \verb|HTTPS|. V~tomto případě by klienti komunikovali se sytémem pomocí \verb|HTTP| metod jako například \verb|get| nebo \verb|post|.
...
\verb|HTTPS| se také postará o~kryptografické zabezpeční přenášených dat, je však nutné získat certifikát k~ověření pravosti webové stránky. 
...
Certifikát pro provoz \verb|HTTPS| bude potřeba zajistit i v~případě, že komunikace s~klienty nebude postavena na tomto protokolu. Je totiž nutné také zabezpečit webové rozhraní, například kvůli ověření identity uživatele.

API systému realizované pomocí \verb|HTTPS| je poměrně snadno rozšiřitelné.
